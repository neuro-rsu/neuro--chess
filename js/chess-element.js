...
const urlCHESS = import.meta.url;
...
export class ChessElement extends LitElement {
    ...
    connectedCallback() {
        ...
        if (this.$$ && this.__globals) {
            CHESS.$$.listen(this.fnGlobals);
            this.__globals.forEach(i => {
                if (CHESS.$$[i] === undefined) CHESS.$$[i] = this[i];
                else this[i] = CHESS.$$[i];
            });
    ...
    disconnectedCallback() {
        ...
        if (CHESS.$$ && this.__globals) CHESS.$$.unlisten(this.fnGlobals);
    ...
    _initBus() {
        if (this.$partid || this.$properties.get('$partid') || (!this.$$ && (this.$properties.get('_partid') || this.__saves || !this.$root || this.__locals || this.__globals))) {
            ...
            if (!CHESS._$$[this._partid]) {
                CHESS._$$[this._partid] = { _$$: {}, _$$: {} };
                CHESS._$$[this._partid]._$$ = icaro({});
                CHESS._$$[this._partid]._$$.update = icaro({ value: 0 });
            }
        }
    }
    ...
    get $$() { return CHESS._$$?.[this.partid]?.['_$$'] ? CHESS._$$[this.partid]['_$$'] : undefined }
        ...
        for (const prop of changedProps.keys()) {
            ...
            if (this.__isFirstUpdated) {
                ...
                if (this.$$ && this.__globals && this.__globals.includes(prop))
                    CHESS.$$[prop] = this[prop];
                // if (CHESS._changed) console.log('_changed', this.localName, prop, { old: changedProps.get(prop) }, { new: this[prop] });
                // if (CHESS._$update) ++this.$$.__changed;
            }
            if (this.__notifications && this.__notifications.has(prop)) {
                ...
                //if (CHESS._notify) console.log('_notify ', this.localName, event, { old: changedProps.get(prop) }, { new: this[prop] });
            }
        }
    }

    $update(property, value) { CHESS.$update.call(this, property, value) }
    $listen(event, fn) {
        if (!event) return;
        CHESS.$$[event] = CHESS.$$[event] || icaro({ count: 0 });
        CHESS.$$[event].listen(fn || this.fnListen);
    }
    $unlisten(event, fn) {
        if (!event || !CHESS.$$[event]) return;
        CHESS.$$[event].unlisten(fn || this.fnListen);
        CHESS.$$[event] = undefined;
    }
    $fire(event, value) {
        if (!event) return;
        CHESS.$$[event] = CHESS.$$[event] || icaro({ count: 0 });
        CHESS.$$[event].value = value;
        ++CHESS.$$[event].count;
    }
...
document.addEventListener('mousedown', (e) => CHESS.mousePos = new DOMRect(e.pageX, e.pageY));
if (!window.CHESSRect) {
    window.CHESSRect = function(element) {
        if (element && element.host) element = element.host;
        const pos = element ? element.getBoundingClientRect() : CHESS.mousePos;
        return pos ? {
